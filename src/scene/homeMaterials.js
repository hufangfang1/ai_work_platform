...
export function makeHomeMaterials(renderer) {
  ...
  for(const source of [bark,tarp,wood]){
  ...
  function mapped(map,repeat=[1,1],extra={}) {
    ...
    const m=mat('#ffffff',{map:copy,bumpMap:bump??null,roughnessMap:rough??null,roughness:rough?1:.95,bumpScale:map===brick?.022:map===ground?.017:map===roofMetal?.005:map===plaster?.015:.009,...extra})
    m.userData.source=map;m.userData.unit=map===brick?[1.04,.64]:map===wood?[1,3]:map===roofMetal?[2,3]:[4,4];return m
  }
  ...
  return {mat,mapped,writing,photoSurface,maps:{plaster,brick,ground,bark,tarp,wood,roofMetal},dispose(){textures.forEach(t=>t.dispose());materials.forEach(m=>m.dispose())}}
}
