...
use app\service\AiDev\GenerationExecutorService;
use app\service\AiDev\RunService;
use app\service\AiDev\TaskService;
use think\queue\Job;

class AiDevGenerationJob
{
    public function fire(Job $job, $data)
    {
        ...
        try {
            ...
            $runService->appendLog($runId, 'queue', 'Worker 领取 AI 生成任务');
            (new GenerationExecutorService())->execute($runId);
            $job->delete();
        } catch (\Throwable $e) {
            $run = $runService->detail($runId);
            ...
            if ($run && $run['run_type'] === 'ai_review' && (int) $run['task_id'] > 0) {
