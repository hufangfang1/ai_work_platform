...
export function makeHomeMaterials(renderer) {
  ...
  function canvasMap(draw, size=1024, colorSpace=T.SRGBColorSpace) {
    ...
    t.wrapS=t.wrapT=T.RepeatWrapping;t.anisotropy=Math.min(16,renderer.capabilities.getMaxAnisotropy());textures.add(t);return t
  }
  ...
  relief.set(plaster,fieldMap((u,v)=>128+plasterGrain(u,v)*24+plasterWear(u,v)*4,1024,true))
  roughness.set(plaster,fieldMap((u,v)=>239+plasterWear(u,v)*10,1024,true))
  ...
  function brickMap(kind) {
    const size=kind==='color'?2048:1024
    return canvasMap((c,s)=>{
  ...
  relief.set(ground,fieldMap((u,v)=>128+groundGrain(u,v)*29+groundPatches(u,v)*5,1024,true))
  roughness.set(ground,fieldMap((u,v)=>239+groundCloud(u,v)*10+groundGrain(u,v)*5,1024,true))
  ...
  relief.set(soil,fieldMap((u,v)=>128+soilGrain(u,v)*32+soilPatches(u,v)*9,1024,true))
  roughness.set(soil,fieldMap((u,v)=>245+soilCloud(u,v)*6+soilGrain(u,v)*4,1024,true))
  ...
  relief.set(roofMetal,fieldMap((u,v)=>128+metalPits(u,v)*13+metalStreak(u,v)*4,1024,true))
  roughness.set(roofMetal,fieldMap((u,v)=>215+metalCloud(u,v)*18+metalStreak(u,v)*8,1024,true))
  ...
  relief.set(firedClay,fieldMap((u,v)=>128+clayPores(u,v)*37+clayWear(u,v)*7,1024,true));
  roughness.set(firedClay,fieldMap((u,v)=>238+clayWear(u,v)*11,1024,true));
  ...
  relief.set(gatePaint,fieldMap((u,v)=>128+paintPores(u,v)*15,1024,true));
  roughness.set(gatePaint,fieldMap((u,v)=>214+paintWear(u,v)*17+paintStreak(u,v)*8,1024,true));
  ...
  relief.set(wood,fieldMap((u,v)=>128+woodFibres(u,v)*17+woodVarnish(u,v)*3,1024,true));
  roughness.set(wood,fieldMap((u,v)=>216+woodVarnish(u,v)*20,1024,true));
  ...
  for(const source of [bark,tarp]){
    const t=canvasMap((c,s)=>{c.drawImage(source.image,0,0,s,s);const data=c.getImageData(0,0,s,s);for(let i=0;i<data.data.length;i+=4){const a=data.data,v=(a[i]+a[i+1]+a[i+2])/3;a[i]=a[i+1]=a[i+2]=v}c.putImageData(data,0,0)},1024,T.NoColorSpace)
    relief.set(source,t)
  }
